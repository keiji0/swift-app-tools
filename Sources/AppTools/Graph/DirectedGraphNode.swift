...
extension DirectedGraphNode {
    ...
    public var deepTargets: some Sequence<Self> {
        TraverseSequence(self, \Self.targets).dropFirst()
    }
    
    /// 自身を除いた子孫一覧を再帰的に取得するシーケンスパス付きの
    public var deepTargetsWithPath: some Sequence<([ID], Self)> {
        TraverseSequenceWithPath(self, \Self.targets).dropFirst()
    }
    
    /// 子孫パス一覧を取得
    /// 自分自身をルートとして含まれる
    public var descendantPaths: some Sequence<[ID]> {
        TraverseSequenceWithPath(self, \Self.targets).lazy.map{
            $0.0 + $0.1.id
        }
    }
